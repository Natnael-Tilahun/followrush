import React, { useEffect, useState, useContext } from "react";
import { Context } from "../context/context";
import { PayPalButton } from "react-paypal-button-v2";

export default function CheckoutForm(processToken) {
  const context = useContext(Context);
  const [locAmount, setLocAmount] = useState("300");
  const [message, setMessage] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
<<<<<<< HEAD
 

  useEffect(() => {
  
=======
  const processId = sessionStorage.getItem("processId");
  // const processToken = sessionStorage.getItem("processToken");
  // const stripe = useStripe();
  // const elements = useElements();

  useEffect(() => {
    if (!processId && !processToken) {
      return;
    }
>>>>>>> ed165ac8d1b6472f367b11f2e90cb4853e99bce8

    // stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
    //   switch (paymentIntent.status) {
    //     case "succeeded":
    //       setMessage("Payment succeeded!");
    //       break;
    //     case "Processing":
    //       setMessage("Your payment is processing.");
    //       break;
    //     case "requires_payment_method":
    //       setMessage("Your payment was not successful, please try again.");
    //       break;
    //     default:
    //       setMessage("Something went wrong.");
    //       break;
    //   }
    // });

    // create new payment process
    try {
      const response = fetch(
        "https://50c15484-d0d9-458a-ae13-a94ac751cbbf.mock.pstmn.io/createPaymentProcess",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: userId,
            sum: amount,
            transactionId: processId,
            transactionToken: processToken,
          }),
        }
      );

<<<<<<< HEAD
=======
      if (!response.ok) {
        throw new Error(`Http error! status: ${response.status}`);
        setMessage(response.status);
      }
      const suspendedTransaction = response.json();
      sessionStorage.setItem(
        "transactionId",
        suspendedTransaction.transactionId
      );
      sessionStorage.setItem(
        "transactionToken",
        suspendedTransaction.transactionToken
      );
      sessionStorage.setItem("asmachta", suspendedTransaction.asmachta);

      return suspendedTransaction;
    } catch (e) {
      //Catch any error and return a status 500
      if (e.code !== "resource_missing") {
        const errorMessage =
          e instanceof Error ? e.message : "Internal server error";
        // res.status(500).json({ statusCode: 500, message: errorMessage });
        return;
      }
    }
>>>>>>> ed165ac8d1b6472f367b11f2e90cb4853e99bce8
  }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();

    if (
      !sessionStorage.getItem("transactionId") ||
      !sessionStorage.getItem("transactionToken")
    ) {
      console.log("not loaded");
      // Stripe.js has not yet loaded.
      return;
    }

    setIsLoading(true);

    // const { error } = await stripe.confirmPayment({
    //   elements,
    //   confirmParams: {
    //     return_url: "https://followrush.vercel.app/order",
    //     receipt_email: context.orderDetails.email,
    //     payment_method_data: {
    //       billing_details: {
    //         name: context.orderDetails.email,
    //       },
    //     },
    //   },
    // });

    fetch("https://smmfollows.com/api/v2", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        pageCode: pageCode,
        transactionId: sessionStorage.getItem("transactionId"),
        transactionToken: sessionStorage.getItem("transactionToken"),
      }),
    })
      .then((res) => {
        res.json();
      })
      .then((data) => {
        if (data.status == "paid") {
          return data;
        }
      })
      .catch((error) => {
        setMessage(error.message);
      });

    setIsLoading(false);
  };

  return (
    <>
      <form
        id="payment-form"
        onSubmit={handleSubmit}
        className="m-auto w-[90%] "
      >
<<<<<<< HEAD
        <PayPalButton id="payment-element" />
        <button
          className="elements-style-background mt-10 w-full bg-[#ffce1e] p-4  rounded-md"
          disabled={isLoading}
=======
        <button
          className="elements-style-background mt-10 w-full bg-[#ffce1e] p-4  rounded-md"
          disabled={
            isLoading ||
            !sessionStorage.getItem("transactionId") ||
            !sessionStorage.getItem("transactionToken")
          }
>>>>>>> ed165ac8d1b6472f367b11f2e90cb4853e99bce8
          id="submit"
        >
          {/* <span id="button-text " className=" "> */}
          {isLoading ? (
            // < className="spinner" id="spinner">
            <span>
              <svg
                role="status"
                className="inline text-center w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                viewBox="0 0 100 101"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                  fill="currentColor"
                />
                <path
                  d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                  fill="currentFill"
                />
              </svg>
              Processing...
            </span>
          ) : (
            "Pay now"
          )}
          {/* </span> */}
        </button>
        {/* Show any error or success messages */}
        {/* {message && <div id="payment-message">{message}</div>} */}
        {message && (
          <div
            className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mt-2 rounded relative"
            role="alert"
          >
            <span className="block sm:inline">{message}</span>
            <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
              <svg
                className="fill-current h-6 w-6 text-red-500"
                role="button"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
              >
                <title>Close</title>
                <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" />
              </svg>
            </span>
          </div>
        )}
      </form>
    </>
  );
}
